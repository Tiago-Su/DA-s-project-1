...
template <class T>
void Adapter<T>::convert_to_graph(Parser parser){
    ...
    for(int i = 3 + parser.reviewers.size(); i < parser.reviewers.size() + parser.submissions.size() + 3; i++){
        ...
        if (parser.parameters.primarySub){
            ...
            for (int vertexid : reviewers_expertise[primary]) graph.addEdge(vertexid, i, 1);
        }
        if (parser.parameters.secondarySub) {
            int secondary = parser.submissions[i - 3 - parser.reviewers.size()].secondary;

            for (int vertexid : reviewers_expertise[secondary]) graph.addEdge(vertexid, i, 1);
        }
...
template <class T>
Adapter<T>::Adapter(Parser parser){
